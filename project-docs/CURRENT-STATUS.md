...
- Property tours MVP 2026-05-04: quedó habilitado el modelo versionado `property_tours/property_tour_items`, políticas públicas seguras para recorridos habilitados, ruta pública `/w/{workspaceSlug}/tours/{tourSlug}` y creación desde cada lead en `/admin/leads` seleccionando propiedades activas. `npm run lint` + `npm run build` pasan; migración pública `20260505055500_public_property_tours.sql` aplicada al remoto.
