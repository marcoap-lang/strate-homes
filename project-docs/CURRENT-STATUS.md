...
## Qué está en curso
- Reentrada 2026-05-05: migración Supabase de colaboradores aplicada al remoto con `npx supabase db push`. Se agregó eliminación segura/desactivación de asesor desde Equipo, edición de correo en asesores sin acceso, sección en Inicio para editar datos principales de la inmobiliaria pública y logo destacado en home pública. `npm run lint` + `npm run build` pasan.
- Asesores múltiples 2026-05-05: se agregó modelo `property_agent_assignments` para colaboradores de propiedad; `properties.agent_id` queda como asesor principal. La sección de editar propiedad ahora permite seleccionar varios asesores y marcar principal; la ficha pública muestra principal + colaboradores; página pública de asesor incluye propiedades donde participa como principal o colaborador. `npm run lint` y `npm run build` pasan.
- Alineación estructural 2026-05-05: Marco definió que el modelo público correcto es inmobiliaria/workspace → asesores → propiedades. El workspace debe presentarse públicamente como inmobiliaria premium; cada propiedad tiene 1 asesor principal responsable comercial y colaboradores opcionales; el WhatsApp principal de propiedad debe ir al asesor principal. Primer pase aplicado: navegación de Área Pública renombrada a Perfil inmobiliaria, Asesores públicos, Propiedades públicas y Vista del sitio; ficha pública marca “Asesor principal”; vistas admin refuerzan WhatsApp/asesor principal y evitan ownership múltiple. `npm run lint` + `npm run build` pasan.
- Reentrada 2026-05-04: se releyeron tracker/status, se detectó que `TODO-NEXT` y `ARCHITECTURE` estaban atrasados frente al código, y se reconciliaron con leads, branding público y el estado histórico de property tours antes de su activación MVP. Después se corrigieron warnings de lint por `_prevState` no usado en server actions y `npm run lint` + `npm run build` pasan correctamente.
- Corrección de alcance 2026-05-04: Marco aclaró que property tours todavía no estaba desarrollado; después pidió dejarlo listo como módulo incremental.
- Primer pase de admin móvil aplicado 2026-05-04: navegación superior sticky en mobile, sidebar solo desktop, contenedor más compacto, cards/formularios/botones más cómodos para celular y corrección de estado activo en navegación. También se ajustó el wizard de propiedades para móvil: la lista larga de pasos se reemplazó por selector compacto con progreso, se agregó botonera sticky inferior y se hicieron más cómodas las acciones de galería/fotos. Después se simplificó el acceso sin sesión: `/admin` redirige a `/login`, el login queda enfocado solo en autenticación y la landing temporal del SaaS en `/` se rediseñó con una presentación más profesional. Se corrigió un blocker del wizard: amenidades/atributos ahora usan estado controlado persistente, la descripción generada se guarda explícitamente en el borrador y los botones de guardar/publicar se desbloquean aunque se repita una validación. También se corrigió el upload de foto de asesor: ahora usa el workspace activo como prefijo de Storage para cumplir las políticas RLS del bucket. Se corrigió además el guardado desde pasos internos del wizard: ahora se mandan campos base persistidos aunque no estén visibles en el paso actual, evitando errores falsos como “título mínimo”, y la edición dejó de partir el wizard junto a la galería en desktop. `npm run lint` + `npm run build` pasan.
- Revisión pública/links 2026-05-04: la producción `homes.strate.lat/properties`, `/w/sarita-homes`, `/w/sarita-homes/properties`, `/w/sarita-homes/properties/casa-costa-de-oro` y la ruta legacy `/properties/casa-costa-de-oro` ya fueron validadas en producción/incógnito. Se aplicó en Supabase remoto la migración pública segura `20260505035000_public_published_inventory_policies.sql`, exponiendo solo propiedades `active` con `published_at`, imágenes de esas propiedades y agentes `is_public + is_active`. La ruta legacy redirige a la canónica por workspace.
- Property tours MVP 2026-05-04: quedó habilitado el modelo versionado `property_tours/property_tour_items`, políticas públicas seguras para recorridos habilitados, ruta pública `/w/{workspaceSlug}/tours/{tourSlug}`, creación desde cada lead en `/admin/leads` y creación sin lead desde `/admin/public/properties` seleccionando propiedades activas. `npm run lint` + `npm run build` pasan; migraciones públicas `20260505055500_public_property_tours.sql` y `20260505065000_property_tours_optional_lead.sql` aplicadas al remoto.
...
8. validar en producción la creación de un recorrido real desde leads y su apertura pública en incógnito
