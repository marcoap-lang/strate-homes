...
- Revisión pública/links 2026-05-04: la producción `homes.strate.lat/properties` y `/w/sarita-homes/properties` cargan pero no muestran inventario en incógnito porque las políticas RLS públicas todavía no permiten leer propiedades publicadas, imágenes ni agentes públicos. Se preparó una migración segura para exponer solo propiedades `active` con `published_at`, imágenes de esas propiedades y agentes `is_public + is_active`. Aplicarla al remoto requiere autorización explícita.
